...
from mpc_solar.pytypes import PythonMsg
...
class BaseConvexObstacle(BaseObstacle):
    ...
    Ab: np.ndarray = field(default = None)  # single ended constraints
...
class RectangleObstacle(BaseConvexObstacle):
    ...
    def __calc_l_A_u__(self):
        A = np.eye(2) @ self.R()
        l = (scipy.linalg.inv(A.T) @ np.array([[self.xc],[self.yc]])).squeeze() - np.array([self.w/2, self.h/2])
        u = (scipy.linalg.inv(A.T) @ np.array([[self.xc],[self.yc]])).squeeze() + np.array([self.w/2, self.h/2])
